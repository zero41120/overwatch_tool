...
export type JunoBreakpoint = JunoCandidate & { budget: number };

export function computeJunoTorpedoBreakpoints(
  enabledItems: JunoItem[],
  opts: JunoOptions = {},
): JunoBreakpoint[] {
  ...
  const bestAtCost = new Map<number, JunoCandidate>();
  ...
  function better(a: JunoCandidate | undefined, b: JunoCandidate) {
    if (!a) return true;
  ...
  function evalChosen() {
    ...
    const cand: JunoCandidate = { cost, damage, ap, baseAdd, burn: hasBurn, names, survival };
  ...
  // starting best at minCash
  let startBest: JunoCandidate | undefined;
  ...
  const rows: JunoBreakpoint[] = [];
  if (startBest) rows.push({ budget: minCash, ...startBest });

  // sweep upward for improvements
