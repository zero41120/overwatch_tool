  ...
  return (
    <div className={`relative block ${className}`} ref={dropdownRef}>
      ...
      >
        ...
        <button
          type="button"
          className="flex-shrink-0 px-3 py-2 text-sm font-medium dark:text-gray-300 transition-colors hover:bg-gray-50 hover:text-gray-900 focus:relative"
          aria-label="Menu"
          onClick={() => setIsOpen(!isOpen)}
        >
          <svg
            ...
            <div>
              ...
              {options.map((option) => (
                <a
                  key={option.value}
                  href="#"
                  className="block px-3 py-2 text-sm font-medium text-gray-900 dark:text-gray-200 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white"
                  role="menuitem"
                  onClick={(e) => {
                    e.preventDefault();
