    ...
    <div>
      ...
      <ul className="mt-2 space-y-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-3 max-h-48 overflow-y-auto">
        {alternatives.map((alt, i) => (
          <li
            key={i}
            className="text-sm text-gray-700 dark:text-gray-300 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
          >
            <strong>Cost: {alt.cost}</strong> -{" "}
            {alt.items.map((it, idx) => (
              <span
                key={it.id || it.name}
                style={{ color: rarityColor(it.rarity) }}
              >
                {it.name}
                {idx < alt.items.length - 1 ? ", " : ""}
              </span>
            ))}
          </li>
        ))}
